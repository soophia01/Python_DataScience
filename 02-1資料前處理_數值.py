...
# 也可以這樣寫: 
# df2[null_columns_mean] = df2[null_columns_mean].fillna(df2[null_columns_mean].mean())
# ---------------------------------------
...
# train_label
price_per_ping = pd.Series(train_label.iloc[:, 1]) # 轉成series
...
# 對數縮放
# =============================================================================
int_columns_log = ['土地', '建物', '車位', '土地移轉總面積(平方公尺)', '建物移轉總面積(平方公尺)', 'nearest_tarin_station_distance', 'income_avg', 'income_var', 'low_use_electricity',
                   '建物現況格局-廳',
                   '建物現況格局-房',
                   '建物現況格局-衛',]
...
for column in int_columns_log:
    ...
    df.insert(loc = col_index, column = column+'_log', value = data_log1p)
    # 轉換後的圖表
    ...
    sns.distplot(pd.Series(df[column+'_log']))
...
# 再取一次 log
# =============================================================================
int_columns_log = ['土地_log', '建物_log', '車位_log', '土地移轉總面積(平方公尺)_log', 'income_avg_log', 'income_var_log',
                   '建物現況格局-廳_log',
                   '建物現況格局-房_log',
                   '建物現況格局-衛_log',]
...
for column in int_columns_log:
    ...
    col_index = df.columns.get_loc(column)
    df = df.drop(columns = column)
    df.insert(loc = col_index, column = column+'_2', value = data_log1p)
    # 轉換後的圖表
    ## 檢查數值中是否包含 NaN 或無限大的數值，因為這些欄位裡面有缺失值還沒處理，會報錯(已先處理)
    # has_nan = np.isnan(data_log1p).any()
    # has_inf = np.isinf(data_log1p).any()
    sns.distplot(pd.Series(df[column+'_2']))
    plt.xlim(data_log1p.min(), data_log1p.max())
    plt.show()
# =============================================================================
...
for column in int_columns_norm:
    ...
    col_index = df.columns.get_loc(column)
    df = df.drop(columns = column)
    df.insert(loc = col_index, column = column+'_norm', value = data_norm)
    # 轉換後的圖表
    sns.distplot(pd.Series(df[column+'_norm']))
    plt.show()
# =============================================================================
...
#     col_index = df.columns.get_loc(column)
#     # df = df.drop(columns = column)
#     df.insert(loc = col_index, column = column+'_sd', value = data_sd)
#     # 轉換後的圖表
#     sns.distplot(pd.Series(df[column+'_sd']))
